...
// /**
//  * FINAL MERGED VERSION – File 1 + File 2
//  * Contains:
//  * ✔ Search
//  * ✔ Locate me
//  * ✔ User marker with pulse animation
//  * ✔ Pollution markers + updating + heat layer
//  * ✔ Eco markers + nudging
//  * ✔ Map click selection (from File 2)
//  * ✔ Parent callback support (File 2)
//  * ✔ Context support for selectedLocation (File 2)
//  * ✔ Clean cleanup (best from File 2)
//  */
...
//           border-radius: 8px;
//         }
//         .lm-popup-title { font-weight: 700; }
//         @keyframes ecoPulse {
//           0% { transform: scale(0.9); opacity: 0.9; }
//           50% { transform: scale(1.25); opacity: 0.45; }
//           100% { transform: scale(1.4); opacity: 0; }
...
//     function popupHTML(title, aqi) {
//       if (aqi == null)
//         return `<div class="lm-popup-card"><div>${title}</div></div>`;
//       return `<div class="lm-popup-card"><div style="display:flex;justify-content:space-between">
//                 <div>${title}</div><strong>AQI ${aqi}</strong>
//               </div></div>`;
//     }
...
//     function createPollutionElement(aqi, catLabel, color) {
...
//       svg.setAttribute("width", "22");
//       svg.setAttribute("height", "22");
//       svg.innerHTML = `<circle cx="12" cy="12" r="11" fill="#E6FFFA"/>`;
...
//       pulse.style.height = "44px";
//       pulse.style.borderRadius = "50%";
//       pulse.style.animation = "ecoPulse 2.5s infinite";
...
//           element: createEcoUserElement(),
//           anchor: "center",
//         })
//           .setLngLat([lng, lat])
//           .setPopup(new tt.Popup().setHTML(popupHTML("You are here", accuracy)))
//           .addTo(map.current);
//       } else {
//         userMarker.current.setLngLat([lng, lat]);
//       }
...
//     // ---------------- HEATMAP SETUP ----------------
//     function buildHeatLayerIfMissing() {
//       if (map.current.getSource("pollution-data")) return;
//       map.current.addSource("pollution-data", {
//         type: "geojson",
//         data: { type: "FeatureCollection", features: [] },
//       });
...
//         source: "pollution-data",
//         paint: {
//           "circle-color": ["interpolate", ["linear"], ["get", "aqi"], 0, "#10b981", 300, "#7f1d1d"],
//           "circle-radius": ["interpolate", ["linear"], ["get", "aqi"], 0, 6, 300, 36],
//           "circle-opacity": 0.42,
//         },
//       });
//     }
...
//     // ---------------- POLLUTION MARKERS ----------------
//     function seedPollutionMarkers(cx, cy, count = 18) {
//       pollutionMarkers.current.forEach((m) => m.marker.remove());
//       pollutionMarkers.current = [];
...
//         const radius = 0.002 + Math.random() * 0.028;
//         const lng = cx + Math.cos(angle) * radius;
//         const lat = cy + Math.sin(angle) * radius;

//         const aqi = Math.floor(60 + Math.random() * 200);
//         const cat = getAQICategory(aqi);

//         const marker = new tt.Marker({
//           element: createPollutionElement(aqi, cat.label, cat.color),
//           anchor: "center",
//         })
//           .setLngLat([lng, lat])
//           .setPopup(new tt.Popup().setHTML(popupHTML(cat.label, aqi)))
//           .addTo(map.current);
...
//       const src = map.current.getSource("pollution-data");
//       if (src) src.setData({ type: "FeatureCollection", features });
//     }

//     function updatePollutionMarkersAndSource() {
//       const features = [];
//       pollutionMarkers.current.forEach((entry) => {
//         const { marker, meta } = entry;

//         let newAQI = Math.max(5, Math.min(350, meta.aqi + (Math.random() - 0.5) * 20));
//         meta.aqi = newAQI;

//         const cat = getAQICategory(newAQI);
//         const el = createPollutionElement(newAQI, cat.label, cat.color);
//         marker.setElement(el);

//         features.push({
//           type: "Feature",
//           properties: { aqi: newAQI },
...
//     // ---------------- ECO MARKERS ----------------
//     function seedEcoMarkers(cx, cy, count = 6) {
//       ecoMarkers.current.forEach((m) => m.remove());
//       ecoMarkers.current = [];
...
//         const marker = new tt.Marker({
//           element: createEcoElementSmall(),
//           anchor: "center",
//         })
//           .setLngLat([lng, lat])
//           .setPopup(new tt.Popup().setHTML(popupHTML("Eco Spot", null)))
//           .addTo(map.current);
...
//     // ---------------- START COMBO UPDATER ----------------
//     function startPollutionAndEcoUpdater(cx, cy) {
//       buildHeatLayerIfMissing();
//       seedPollutionMarkers(cx, cy);
//       seedEcoMarkers(cx, cy);

//       if (pollutionUpdaterRef.current) clearInterval(pollutionUpdaterRef.current);

//       pollutionUpdaterRef.current = setInterval(() => {
//         if (Math.random() < 0.15) {
//           seedPollutionMarkers(cx + (Math.random() - 0.5) * 0.01, cy + (Math.random() - 0.5) * 0.01);
//         } else {
//           updatePollutionMarkersAndSource();
//         }

//         if (Math.random() < 0.5) lightlyNudgeEcoMarkers();
//       }, 4000);
//     }

//     // ---------------- MAP MOVE / RESEED ----------------
//     function shouldReseed(oldC, newC) {
//       if (!oldC) return true;
//       return Math.abs(oldC[0] - newC[0]) > 0.004 || Math.abs(oldC[1] - newC[1]) > 0.004;
//     }

//     function onMapMoveEnd() {
//       const c = map.current.getCenter();
//       const newC = [c.lng, c.lat];

//       if (shouldReseed(lastReseedCenter.current, newC)) {
//         lastReseedCenter.current = newC;
//         startPollutionAndEcoUpdater(c.lng, c.lat);
//         isFollowingUserRef.current = false;

//         if (onCenterChange) onCenterChange({ lat: c.lat, lng: c.lng });
//       }
//     }
...
//       map.current.flyTo({ center: [res.lon, res.lat], zoom: 13 });

//       if (searchResultMarker.current) searchResultMarker.current.remove();
...
//           el.style.width = "26px";
//           el.style.height = "34px";
//           el.style.background = "linear-gradient(#0ea5a4,#028488)";
//           el.style.borderRadius = "6px";
//           el.style.color = "white";
//           el.style.display = "flex";
//           el.style.alignItems = "center";
...
//         .setPopup(new tt.Popup().setHTML(popupHTML(res.address)))
//         .addTo(map.current);

//       searchResultMarker.current = marker;

//       lastReseedCenter.current = [res.lon, res.lat];
//       startPollutionAndEcoUpdater(res.lon, res.lat);

//       if (onCenterChange) onCenterChange({ lat: res.lat, lng: res.lon });
//       if (setSelectedLocation)
//         setSelectedLocation({
//           name: res.address,
//           latitude: res.lat,
...
//       if (setSelectedLocation)
//         setSelectedLocation({
//           name: `Selected: ${lng.toFixed(4)}, ${lat.toFixed(4)}`,
//           latitude: lat,
//           longitude: lng,
//         });
...
//       try {
//         input.removeEventListener("keydown", runSearchOnce);
//         locateBtn.removeEventListener("click", runSearchOnce);
//       } catch {}

//       if (controlWrapper.parentNode) controlWrapper.parentNode.removeChild(controlWrapper);
...
//       if (map.current) {
//         map.current.off("moveend", onMapMoveEnd);
//         map.current.off("click", onMapClick);

//         if (map.current.getLayer("pollution-heat"))
//           map.current.removeLayer("pollution-heat");
...
export function LiveMap({ onLocationChange, onCenterChange, center }) {
  ...
  useEffect(() => {
    ...
    function popupHTML(title, aqi) {
      if (aqi == null)
        return `<div class="lm-popup-card"><div class="lm-popup-title">${title}</div></div>`;
      return `<div class="lm-popup-card"><div style="display:flex;align-items:center;justify-content:space-between">
                <div class="lm-popup-title">${title}</div>
                <div class="lm-popup-aqi">AQI <strong style="margin-left:6px">${aqi}</strong></div>
              </div></div>`;
    ...
    function createEcoUserElement() {
      ...
      pulse.style.boxShadow =
        "0 0 0 4px rgba(16,185,129,0.12) inset, 0 0 12px rgba(16,185,129,0.12)";
      pulse.style.animation = "ecoPulse 2.5s ease-out infinite";
      ...
      shine.setAttribute(
        "d",
        "M24 22c1.2-2.4 4.2-3.6 6.6-2.4 0 0-2.8 1.2-4 3.6-1.2 2.4-3 1.2-2.6-1.2z"
      );
      shine.setAttribute("fill", "rgba(255,255,255,0.55)");
      svg.appendChild(shine);
    ...
    function updateUserMarker(lng, lat, accuracy) {
      ...
      if (!userMarker.current) {
        ...
        userMarker.current = new tt.Marker({ element: el, anchor: "center" })
          .setLngLat([lng, lat])
          .setPopup(
            new tt.Popup({ offset: 10 }).setHTML(
              popupHTML("You are here", Math.round(accuracy ?? 0))
            )
          )
          .addTo(map.current);
      } else {
      ...
      // update context with current user location
      try {
        if (setSelectedLocation) {
          setSelectedLocation({
            name: "Your location",
    ...
    function seedPollutionMarkers(centerLng, centerLat, count = 18) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const marker = new tt.Marker({ element: el, anchor: "center" })
          .setLngLat([lng, lat])
          .setPopup(
            new tt.Popup({ offset: 8 }).setHTML(popupHTML(cat.label, aqi))
          )
          .addTo(map.current);
      ...
      const src =
        map.current.getSource && map.current.getSource("pollution-data");
      if (src) {
        try {
          src.setData({ type: "FeatureCollection", features });
        } catch (e) {}
    ...
    function updatePollutionMarkersAndSource() {
      ...
      pollutionMarkers.current.forEach((entry, idx) => {
        ...
        const regionalBias = 6 * Math.sin((meta.lng + meta.lat) * 12);
        newAQI = Math.round(
          Math.max(5, Math.min(350, newAQI + regionalBias))
        );
        meta.aqi = newAQI;
        const cat = getAQICategory(newAQI);
    ...
    function seedEcoMarkers(centerLng, centerLat, count = 6) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const marker = new tt.Marker({ element: el, anchor: "center" })
          .setLngLat([lng, lat])
          .setPopup(
            new tt.Popup({ offset: 8 }).setHTML(popupHTML("Eco Spot", null))
          )
          .addTo(map.current);
      ...
      if (shouldReseed(last, [centerLng, centerLat], 0.004)) {
        ...
        try {
          if (typeof onCenterChange === "function")
            onCenterChange({ lat: centerLat, lng: centerLng });
        } catch (e) {}
      }
    }
    ...
    if ("geolocation" in navigator) {
      watchIdRef.current = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // ✅ save current location for predictions page (from File 1)
          const loc = {
            lat: latitude,
            lon: longitude,
    ...
    async function geocodeSearch(query) {
      ...
      try {
        ...
        const lat = r.position?.lat ?? r.lat ?? null;
        const lon =
          r.position?.lon ?? r.lon ?? r.position?.lon ?? null;
        const address = r.address?.freeform ?? r.poi?.name ?? query;
        if (!lat || !lon) return null;
        return { lat, lon, address };
      } catch (e) {
    ...
    function showSearchResultMarker(lng, lat, title) {
      try {
        ...
        searchResultMarker.current = new tt.Marker({
          element: el,
          anchor: "bottom",
        })
          .setLngLat([lng, lat])
          .setPopup(
            new tt.Popup({ offset: 12 }).setHTML(popupHTML(title, null))
          )
          .addTo(map.current);
      } catch (e) {
        console.error("Search marker failed", e);
    ...
    async function runSearchOnce() {
      ...
      // ✅ save searched location for predictions page (from File 1)
      const loc = {
        lat: res.lat,
        lon: res.lon,
        label: res.address || q,
      ...
      try {
        ...
        console.warn("Could not save search location", e);
      }

      // notify parent about center change because of search
      try {
        if (typeof onCenterChange === "function")
          onCenterChange({ lat: res.lat, lng: res.lon });
      } catch (e) {}
    ...
    function onMapClick(e) {
      try {
        const lngLat =
          e.lngLat ||
          (e.coordinates
            ? { lng: e.coordinates[0], lat: e.coordinates[1] }
            : null);
        if (!lngLat) return;
        const { lng, lat } = lngLat;
        isFollowingUserRef.current = false;
    ...
    return () => {
      ...
      try {
        ...
        input.removeEventListener("keydown", onInputKeyDown);
        locateBtn.removeEventListener("click", onLocateClick);
        if (controlWrapper && controlWrapper.parentNode)
          controlWrapper.parentNode.removeChild(controlWrapper);
      } catch (e) {}
      ...
      try {
        ...
        pollutionMarkers.current.forEach((e) => {
          try {
            e.marker.remove();
          } catch (e) {}
        });
      ...
      // remove heat source & layer + click handler
      try {
        if (map.current) {
          try {
            map.current.off("click", onMapClick);
          ...
          if (
            ...
            map.current.getLayer("pollution-heat")
          )
            map.current.removeLayer("pollution-heat");
          if (
            map.current.getSource &&
            map.current.getSource("pollution-data")
          )
            map.current.removeSource("pollution-data");
          map.current.remove();
        }
...
//     // 🔹 ADDED: helper to persist last selected location for Predictions page (non-invasive)
//     function persistLocation(label, latitude, longitude) {
//       if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return;
//       const loc = { lat: latitude, lon: longitude, label };
//       try {
//         localStorage.setItem("geoSense:lastLocation", JSON.stringify(loc));
//       } catch (e) {
//         // won't break anything if storage fails
//         console.warn("Could not save location for predictions", e);
//       }
//     }
...
//       // 🔹 ADDED: persist last user location for Predictions (non-invasive)
//       try {
//         persistLocation("Your location", lat, lng);
//       } catch (e) {
//         // ignore errors
//       }
//     }
...
//           // 🔹 ADDED: persist last live location for predictions (non-invasive)
//           try {
//             persistLocation("Your location", latitude, longitude);
//           } catch (e) {}
//         },
//         (error) => {
//           console.error("⚠️ Geolocation error:", error.message);
...
//       // 🔹 ADDED: persist searched location for predictions (non-invasive)
//       try {
//         persistLocation(res.address || q, res.lat, res.lon);
//       } catch (e) {}
//     }
...
//         // notify parent about location (user located)
//         try {
//           if (typeof onLocationChange === "function") onLocationChange({ lat: lat, lng: lng });
//         } catch (e) {}
//         // also notify center change
//         try {
//           if (typeof onCenterChange === "function") onCenterChange({ lat: lat, lng: lng });
//         } catch (e) {}
...
//         // 🔹 ADDED: persist located location for predictions
//         try {
//           persistLocation("Your location", lat, lng);
//         } catch (e) {}
//       } else {
...
//         // 🔹 ADDED: persist clicked location for predictions
//         try {
//           persistLocation(`Selected: ${lng.toFixed(4)}, ${lat.toFixed(4)}`, lat, lng);
//         } catch (e) {}
